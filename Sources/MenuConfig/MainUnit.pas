...
procedure TMainForm.SaveAllTBClick(Sender: TObject); //сохранить всЄ
var
  i, j: Integer;
  SectorName: String;
begin
  for i := 0 to PageControl1.PageCount - 1 do
    begin
      ...
      Settings.WriteInteger('Monitor' + IntToStr(i), 'SectorsCount', ((PageControl1.Pages[i].Controls[0] as TScrollBox).Controls[0] as TPanel).ControlCount); //сохран€ем количество секторов на текущей вкладке
      for j := 0 to ((PageControl1.Pages[i].Controls[0] as TScrollBox).Controls[0] as TPanel).ControlCount - 1 do //цикл по количеству компонентов
        begin
          SectorName := 'Monitor' + IntToStr(i) + 'Sector' + IntToStr(j); //им€ раздела информации о секторе
          with ((PageControl1.Pages[i].Controls[0] as TScrollBox).Controls[0] as TPanel).Controls[j] as TPanel do //конструкци€ вида PageControl1.Pages[i].ScrollBox.Panel.Panel[j]
            begin
